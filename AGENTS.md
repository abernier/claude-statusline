...
`docs/` is a Jekyll site — GitHub Pages builds it, from a branch, path `/docs`. `docs/index.html` is still one hand-written file with inline `<style>` and `<script>`, but it opens with front matter and a block of Liquid assigns, and no repository name is written anywhere below them: the links, the install and uninstall one-liners and the by-hand `raw.githubusercontent` fetches all read `site.github.*`, which `jekyll-github-metadata` fills from the repository the site is built from. The one name spelled out is `canonical`, and only so the page can tell whether it is upstream — a fork adds `--repo <itself>` to every command, because the `install.sh` it serves still carries upstream's built-in default, and says in the sidebar and the footer whose fork it is. Never emit `--ref`: the installer resolves the repository's default branch itself. `docs/_config.yml` names the plugin and turns the default theme off; `docs/Gemfile` pins the `github-pages` gem for the builds that are not GitHub Pages, which have no GitHub context and must set `PAGES_REPO_NWO`, and `JEKYLL_GITHUB_TOKEN` too — `site.github.url` and `site.github.source.branch` come from the Pages API, which refuses anonymous reads.
